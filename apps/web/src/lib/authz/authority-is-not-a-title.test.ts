...
describe("finance authority comes from the bundle the seat was given", () => {
  ...
  it("still lets the president manage it", () => {
    ...
    expect(
      canManageFinance(
        ctx([seat({ scope: "PRESIDENT", templateKey: "unit.lead" })]),
        ORG,
      ),
    ).toBe(true);
  });
  ...
  it("still lets the OSE Director manage it", () => {
    expect(
      canManageFinance(
        ctx([], [{ institutionId: "inst-1", role: "OSE_DIRECTOR" }]),
        ORG,
      ),
    ).toBe(true);
  });
  ...
  it("refuses an ordinary member", () => {
    expect(
      canManageFinance(ctx([seat({ templateKey: "unit.member" })]), ORG),
    ).toBe(false);
  });
  ...
  it("refuses a SHADOW finance officer", () => {
    ...
    expect(canManageFinance(ctx([seat({ status: "SHADOW" })]), ORG)).toBe(
      false,
    );
  });

  it("refuses an ALUMNI finance officer", () => {
    expect(canManageFinance(ctx([seat({ status: "ALUMNI" })]), ORG)).toBe(
      false,
    );
  });
  ...
  it("does not reach into another club", () => {
    expect(
      canManageFinance(ctx([seat({ organizationId: "club-2" })]), ORG),
    ).toBe(false);
  });
});
...
describe("the title no longer decides anything", () => {
  it("gives a seat called Budget Lead the authority it was granted", () => {
    ...
    expect(
      canManageFinance(
        ctx([
          seat({ roleName: "Budget Lead", templateKey: "finance.officer" }),
        ]),
        ORG,
      ),
    ).toBe(true);
  ...
  it("gives a seat called Financial Inclusion Officer nothing it was not granted", () => {
    ...
    expect(
      canManageFinance(
        ctx([
          seat({
            roleName: "Financial Inclusion Officer",
  ...
  it("does not change what somebody may do when their seat is renamed", () => {
    ...
    const before = canManageFinance(
      ctx([seat({ roleName: "Treasurer" })]),
      ORG,
    );
    const after = canManageFinance(
      ctx([seat({ roleName: "Steward of Funds" })]),
      ORG,
    );
...
describe("the finance bundle is read from the catalog, not listed twice", () => {
  it("recognises every template that confers budget editing", () => {
    for (const template of ROLE_TEMPLATES) {
      ...
      expect(carriesFinanceAuthority({ templateKey: template.key })).toBe(
        confers,
      );
    }
  });
  ...
  it("refuses a template key the platform does not ship", () => {
    ...
    expect(carriesFinanceAuthority({ templateKey: "finance.everything" })).toBe(
      false,
    );
