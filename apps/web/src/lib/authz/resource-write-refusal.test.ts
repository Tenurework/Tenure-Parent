...
describe("what the board says when it refuses", () => {
  it("says nothing when the decision allows", () => {
    expect(
      resourceWriteRefusal({ allowed: true, reason: "ALLOWED", detail: "" }, OFFICE, "publish"),
    ).toBeNull()
  })
  ...
  it("names the office when the answer is the wrong role", () => {
    ...
    const message = resourceWriteRefusal(
      { allowed: false, reason: "NO_ROLE_GRANTING", detail: "no role confers it" },
      OFFICE,
      "publish",
    )
  ...
  it("uses the verb it was given", () => {
    for (const verb of ["publish", "edit", "retire"]) {
      expect(resourceWriteRefusal({ allowed: false, reason: "NO_ROLE_GRANTING", detail: "" }, OFFICE, verb)).toContain(
        `can ${verb} board`,
      )
    }
  })
  ...
  it("does not blame the reader's role when the module is off", () => {
    ...
    const detail = '"resources.resource.create" belongs to module "resources", which this system does not run.'
    const message = resourceWriteRefusal(
  ...
  it("falls back to the office for every other denial", () => {
    ...
    for (const reason of ["OUT_OF_SCOPE", "MEMBERSHIP_NOT_ACTIVE", "POLICY_DENIED"] as const) {
      const message = resourceWriteRefusal({ allowed: false, reason, detail: "internals" }, OFFICE, "edit")
