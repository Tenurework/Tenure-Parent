...
const org = { id: CLUB, institutionId: TENANT }

const allows = (ctx: UserContext, permission: string, organizationId?: string) =>
...
describe("every institution role maps to a template", () => {
  it("names one for each value of the enum", () => {
    ...
    expect(Object.keys(INSTITUTION_TEMPLATES).sort()).toEqual([...ROLES].sort())
  })
...
describe("the decision agrees with canManageResources", () => {
  ...
  it.each(ROLES)("agrees for %s", (role) => {
    const ctx = ose(role)
    expect(allows(ctx, "resources.resource.create")).toBe(canManageResources(ctx, TENANT))
  })
  ...
  it("is not vacuous — the three roles do not all answer the same", () => {
    ...
    const answers = ROLES.map((role) => allows(ose(role), "resources.resource.create"))
...
describe("the decision agrees with canManageRoster", () => {
  it.each(ROLES)("agrees for %s", (role) => {
    const ctx = ose(role)
    expect(allows(ctx, "org.roster.update", CLUB)).toBe(canManageRoster(ctx, org))
  })

  it("is not vacuous — only the Director manages a roster", () => {
    const answers = ROLES.map((role) => allows(ose(role), "org.roster.update", CLUB))
...
describe("the decision agrees with canViewOrg and canListAllOrgs", () => {
  ...
  it("is not vacuous — every OSE role sees every unit", () => {
    ...
    expect(ROLES.map((role) => allows(ose(role), "org.unit.read", CLUB))).toEqual([
      true,
      true,
      true,
    ])
...
describe("the decision agrees with isOse on the second approval gate", () => {
  ...
  it.each(ROLES)("agrees for %s", (role) => {
    const ctx = ose(role)
    expect(allows(ctx, "approvals.request.decide", CLUB)).toBe(isOse(ctx, TENANT))
  })

  it("is not vacuous — all three decide, and a non-member does not", () => {
    expect(ROLES.map((role) => allows(ose(role), "approvals.request.decide", CLUB))).toEqual([
      true,
      true,
      true,
    ])
    const outsider: UserContext = { userId: "x", institutionRoles: [], orgRoles: [] }
...
describe("the decision agrees with isOseDirector on budget management", () => {
  it.each(ROLES)("agrees for %s", (role) => {
    const ctx = ose(role)
    expect(allows(ctx, "finance.budget.update", CLUB)).toBe(isOseDirector(ctx, TENANT))
  })
})

describe("somebody with no institution role gets nothing from it", () => {
  const outsider: UserContext = { userId: "u2", institutionRoles: [], orgRoles: [] }
