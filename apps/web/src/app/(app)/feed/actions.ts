...
export async function decideCollab(formData: FormData) {
  ...
  await withTenantScope(userId, async () => {
    ...
    // CollabInterest is UNENFORCEABLE — it carries no institutionId, so the
    // scope cannot filter this read and the isOseDirector check below is still
    // the only thing standing between an id and another institution's request.
    // Fixed by denormalising institutionId onto it, not here.
