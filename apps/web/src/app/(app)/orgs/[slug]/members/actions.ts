 ...
 * "some" change happened). Returns the org and the loaded context.
 *
 * Reads the club and writes the DENY audit row, so it runs inside the caller's
 * tenant scope; the caller has already resolved the session and passes the
 * acting user in.
 */
async function requireRosterManager(userId: string, slug: string) {
...
export async function assignMember(slug: string, formData: FormData) {
  ...
  await withTenantScope(userId, async () => {
    const { org } = await requireRosterManager(userId, slug)
...
export async function transitionAssignment(slug: string, formData: FormData) {
  const userId = await requireUserId()
  await withTenantScope(userId, async () => {
    const { org, ctx } = await requireRosterManager(userId, slug)
