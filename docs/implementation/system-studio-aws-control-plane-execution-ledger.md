...
## STUDIO-010-003 — the OU hierarchy, the guardrails each unit inherits, and the account lifecycle that ends in Closure

- [x] **STUDIO-010-003** — "Define OU hierarchy and inherited guardrails for Security, Infrastructure, Workloads, Suspended, Quarantine, Sandbox, and Closure lifecycle."
  - Status: PASS
  - Code: `apps/system-studio/src/lib/aws/organization-units.ts` — `ORGANIZATIONAL_UNITS` (the seven units, three levels: Security / Infrastructure / Workloads / Sandbox / Suspended under the root, Quarantine and Closure under Suspended), `ROOT_GUARDRAILS`, `Guardrail`, `ancestry`, `inheritedGuardrails`, `deniedActionCount`, `guardrailDefects`, `ACCOUNT_MOVES`, `canMoveAccount`, `TERMINAL_UNIT`, `organizationTree`, `reconcileOrganizationalUnits`, `unitSummary`, `organizationalUnitSurface`.
    Supporting: `apps/system-studio/src/lib/aws/capabilities.ts` — three new read capabilities (`organizations:ListRoots`, `organizations:ListOrganizationalUnitsForParent`, `organizations:ListPoliciesForTarget`); `apps/system-studio/src/lib/aws/client.ts` — their three gateway arms, with `Filter` fixed to `SERVICE_CONTROL_POLICY` at the seam rather than taken from a caller; `infrastructure/studio/iam.tf` — the three List actions on the existing `EstatePosture` statement; `apps/system-studio/src/lib/aws/inventory.ts` — a `notComposedSection` adjudicating them (an OU is not an estate resource).
  - Caller: `apps/system-studio/src/app/platform/estate/page.tsx:219` calls `organizationalUnitSurface()` on every load (`export const dynamic = "force-dynamic"`, line 68) and renders the verdicts as the "Organizational units and inherited guardrails" card — one row per declared unit, each printing its effective guardrail ids, its denied-action count, and whether an SCP is attached at it.
  - Tests: `apps/system-studio/src/lib/aws/organization-units.test.ts` — **30 tests, 30 passing**; `apps/system-studio/src/app/platform/estate/organizational-units-wiring.test.ts` — **5 tests, 5 passing**. The reconciliation is driven through the REAL reader with a stand-in at the `AwsGateway` seam, so a reader that stopped walking the tree reds rather than staying green against a fixture.
  - Evidence:
    - `cd apps/web && npx jest --ci ../system-studio/src/lib/aws/organization-units.test.ts` → `Test Suites: 1 passed, 1 total / Tests: 30 passed, 30 total`
    - `cd apps/web && npx jest --ci ../system-studio/src` → `Test Suites: 86 passed, 86 total / Tests: 2893 passed, 2893 total` (no regression across the whole Studio suite)
    - `node --test tests/security/studio-task-role-is-narrow.test.mjs` → `# pass 11 / # fail 0` — the three new grants are read verbs and stay inside the inline-policy limit
    - `node --test tests/architecture/every-provisioned-service-has-a-reader.test.mjs` → `# pass 9 / # fail 0`
    - `npx tsc --noEmit -p apps/system-studio/tsconfig.json` → no error in any file of this row (6 pre-existing errors remain in `packages/provisioning/src/placement-policy.ts` and `placement-override.ts`, another agent's in-flight work, untouched here)
  - What is NOT closed, and is not claimed: the reconciliation reports what the account holds; it does not create or move an OU, and it must not — `mutate.ts` is the only place a mutation lives and the task role is granted list verbs only. Account vending into these units is STUDIO-010-004 and is untouched. The reader's own live behaviour against a real AWS Organization is unverified here for the same reason every adapter in this directory is: there is no Organization yet (the standing BLOCKED_EXTERNAL gate), which is exactly why every row comes back UNREAD with the pasteable minimum statement rather than as a finding.

## STUDIO-010-005 — the eight roles AWS authority is split across, their boundaries, and their session tags

- [x] **STUDIO-010-005** — "Define separate read, plan, deploy-development, deploy-staging, deploy-production, security-remediation, lifecycle, and break-glass roles with permissions boundaries and session tags."
  - Status: PASS
  - Code: `apps/system-studio/src/lib/aws/deployment-roles.ts` — `DEPLOYMENT_ROLES` (the eight, each with `permissionsBoundary`, a non-empty `sessionTags`, `maxSessionSeconds`, `requiresMfa`, `assumableBy` and a symmetric `separationFrom`), `roleDefects`, `conflictingRoles`, `reconcileDeploymentRoles`, `roleSummary`, and the verdict unions `RolePresence` / `BoundaryVerdict` / `SessionTagVerdict` / `TrustVerdict`.
    Supporting: `apps/system-studio/src/lib/aws/iam.ts` — `IamPrincipal` gained two REQUIRED fields, `permissionsBoundaryArn: string | null` (which boundary, not merely whether there is one — "a boundary is attached" is satisfied by the wrong boundary) and `assumeRolePolicy: PolicyDocumentRead | null` (the decoded trust policy; null for a user, which is signed into rather than assumed). Both come off data `iam:GetAccountAuthorizationDetails` already returns, so this costs no extra call and no extra grant.
  - Caller: `apps/system-studio/src/app/platform/identity/page.tsx:182` calls `reconcileDeploymentRoles({ posture: iam.read })` on every load (`export const dynamic = "force-dynamic"`) and renders the "How AWS authority is split" card — role, whether the account holds it, which boundary caps it, its declared session tags and what its trust policy actually permits. It is fed `iam.read` (the `AwsRead` union) and NOT `iam.posture` (`IamPosture | null`), which would compile and would turn a refused IAM read into eight missing roles; the wiring spec pins that.
  - Tests: `apps/system-studio/src/lib/aws/deployment-roles.test.ts` — **24 tests, 24 passing**, including one that drives the REAL `iamPosture` through an `AwsGateway` stand-in and asserts the two new fields reach the reconciliation; `apps/system-studio/src/app/platform/identity/control-plane-roles-wiring.test.ts` — **5 tests, 5 passing**.
  - Evidence:
    - `cd apps/web && npx jest --ci ../system-studio/src/lib/aws/deployment-roles.test.ts` → `Tests: 24 passed, 24 total`
    - `cd apps/web && npx jest --ci ../system-studio/src/app/platform/identity ../system-studio/src/lib/aws/deployment-roles.test.ts` → `Tests: 70 passed, 70 total`
    - `cd apps/web && npx jest --ci ../system-studio/src` → `Test Suites: 86 passed, 86 total / Tests: 2893 passed, 2893 total`
    - `npx tsc --noEmit -p apps/system-studio/tsconfig.json` → no error in any file of this row
    - The declaration's own guard is not decoration: `roleDefects()` failed on the first run with "lifecycle is separated from deploy-production and deploy-production is not separated from lifecycle", a genuine one-way separation in the draft table, and the fix is in the shipped declaration.
  - What is NOT closed, and is not claimed: **which session-tag keys a trust policy demands cannot be read from this call.** `PolicyStatement` carries `hasCondition` as a boolean and not the condition body, so `SessionTagVerdict` has a `KEYS_UNVERIFIABLE` arm that says so in the operator's own words rather than reporting a pass. What IS proven is the arm that matters — a role whose trust policy does not permit `sts:TagSession` provably cannot carry a session tag at all, and that is reported as `CANNOT_BE_TAGGED`, a finding. Extending `decodePolicyDocument` to carry condition keys would close both that gap and most of STUDIO-010-007. Nothing here creates, updates or assumes a role; the eight are declared and compared, and standing them up in AWS is blocked on the same missing Organization as the rest of Phase 010.

## STUDIO-060-004 — a change's blast radius is calculated on all twelve axes, and the one that cannot be counted says so
...
- [ ] **STUDIO-060-004** — "Calculate blast radius: tenants, users, seats, workflows, modules, records, resources, regions, integrations, SLOs, downtime, and downstream releases."
  - Status: FAIL
  - Overturned on review: Mutations reproduce (M1: 4 failed incl. both named tests; M2: 1 failed 'modules is the changed set plus everything transitively depending on it'; both restored, 94 green). Code is reached: governance.ts:221 blastRadius() <- page.tsx:643 tenantGovernance() -> GovernancePanel.tsx:58 blastRadiusLines(). Eleven axes are genuinely computed off the real @tenure/modules catalogue. REFUTED on step 1 — the requirement says CALCULATE twelve axes and `users` is not calculated, it is a hardcoded constant. (a) blast-radius.ts:234-240 pushes a literal `unreadable(...)` unconditionally; `BlastInput` (l.129-152) has no `users` field at all, unlike `seats`/`cell`/`resources` which are caller-supplied Readings — so the axis can never be answered without changing the type. That is a constant return for a named axis, not an honest-unknown that a caller could fill. (b) The stated justification is false about this codebase: 'this control plane holds no user table ... a user count would have to come from the tenant's own cell'. apps/system-studio/src/lib/aws/cognito.ts:937-938 already reads `EstimatedNumberOfUsers` into `PoolDetail.estimatedUsers`, and `poolAttribution()` (cognito.ts:1477) already resolves a pool to `{kind:'tenant', tenantSlug}` from its tags — both consumed by src/app/platform/identity/page.tsx. A per-tenant user count is a read this console already makes; the `fix` string points at a non-existent 'cell operations API' instead. (c) The cited authority is misattributed: tests/security/no-personal-data.test.mjs forbids real @rochester.edu addresses in git-tracked files (REAL_DOMAINS regex); it says nothing about a user table and would not be violated by a COUNT. Secondary note, not the basis: `downtime` returns a known 0 for every non-lifecycle surface (interruption() l.440 returns null unless surface==='tenant-lifecycle'), and `records` counts object classes rather than records — both labelled, but both proxies for the axis the requirement names.
  - Code: `apps/system-studio/src/lib/change/blast-radius.ts` — `BLAST_DIMENSIONS` (the twelve, in the requirement's order), `blastRadius`, `affectedModules`, `interruption`, `blastRadiusProblems`, `blastRadiusLines`; types `BlastRadius`, `BlastMeasure`, `BlastCount`, `BlastInput`. Assembled for the page by `apps/system-studio/src/app/tenants/[slug]/governance.ts` — `tenantGovernance`, `readingOf`, `regionOf`.
  - Caller: `apps/system-studio/src/app/tenants/[slug]/page.tsx:643` calls `tenantGovernance(...)`; `:1627` renders `<GovernancePanel governance={governance} .../>`. `apps/system-studio/src/components/GovernancePanel.tsx:57` renders `blastRadiusLines(move.blast)`. `governance.ts:206` calls `blastRadius(...)` once per permitted move.
  - What is actually CALCULATED, and from what: `modules` is `dependantsOf` in `src/lib/revisions.ts` — the existing transitive walk, called rather than reimplemented; `workflows` is `PROCESS_CHAINS` from the real `@tenure/modules` catalogue filtered to chains crossing an affected module; `records` is the `objects` (Prisma model names) those modules govern, with the unit stating in words that it counts CLASSES and not rows; `slos` is the `slo` blocks those modules declare; `tenants` is the cell's own `capacity.tenants`; `regions` unions the manifest, the cell and every attributed resource's ARN region; `integrations` is `externalDomains(manifest.systemOfRecord)`; `downtime` is derived from the lifecycle move (leaving a serving state for one that does not serve), NOT from the change class — a domain activation is C6 and interrupts nobody; `downstreamReleases` is the cell's engine release; `seats` is the commercial plan's seat quota; `resources` is the tag-attributed estate.
  - The one axis that is not a number, stated rather than faked: `users`. This control plane holds no user table, deliberately, and `tests/security/no-personal-data.test.mjs` is what keeps it that way. It is reported `known: false` with `because: "this control plane holds no user table — user records live in the tenant's own cell database, which the console does not read"` and `fix: "Add a per-tenant user-count read to the cell's operations API and pass it in; do not copy user rows into the control plane."` A `0` there would be a confident wrong answer about how many people a suspension affects.
  - An axis derived from a failed read FAILS with it, on purpose: `regions` could be answered partially (the manifest and the cell both name one even when the estate read is denied) and is reported unreadable anyway, because a blast radius that understates is the dangerous direction. Same rule ties `tenants` and `downstreamReleases` to the cell reading. Proven: with a denied `dynamodb:Query`, `report.unreadable` contains both `resources` and `regions`, and the regions reading carries the denial's own `fix`.
  - The report checks itself: `blastRadiusProblems` reds an axis that is declared and never measured, and a `count` with a shorter `items` list and no `itemsWithheld` — a list silently shorter than its own count reads as an enumeration when it is a total.
  - Tests: `apps/system-studio/e2e/change-governance-logic.spec.ts` (46 tests, browser-free — Playwright as a plain runner, the `preferences-logic.spec.ts` pattern) and `apps/system-studio/e2e/tenant-governance-logic.spec.ts` (21 tests, over the page's own assembly). Both drive the REAL `MODULES` / `PROCESS_CHAINS` catalogues wherever a property should hold over what ships.
  - Evidence: `cd apps/system-studio && ../../node_modules/.bin/playwright test e2e/change-governance-logic.spec.ts e2e/portability-logic.spec.ts e2e/tenant-governance-logic.spec.ts --reporter=line` → `94 passed (8.7s)`. `npx tsc --noEmit` → 0 errors. `node --test tests/security/no-personal-data.test.mjs tests/architecture/no-fixture-tenants-on-operator-surfaces.test.mjs tests/security/no-hardcoded-estate.test.mjs` → `# pass 10 # fail 0`.
  - Mutation proven: `users` forced to a known `0` → **4 red** (`eleven axes answer and only \`users\` says it could not look`, `\`users\` names the read that would answer it rather than reporting zero`, +2). Restored: 42 green. Separately, `dependantsOf(...)` replaced with `[]` → **1 red** (`modules is the changed set plus everything transitively depending on it`). Restored: 42 green.
  - Honest limit: `users` is the only axis with no number, and the row above says why and what would fix it. Nothing here reads a clock, a network or an environment variable — every input is a parameter, so the report a test renders is the report the page renders.

## STUDIO-060-008 — a change now has a calendar, a freeze that outranks an emergency, a notice that must be recorded, and a review it owes afterwards
...
- [ ] **STUDIO-060-008** — "Implement scheduled change windows, freeze periods, maintenance notifications, cancellation, supersession, and emergency change with after-action review."
  - Status: FAIL
  - Overturned on review: Mutations reproduce exactly as claimed (M3: 1 failed 'a declared emergency does NOT lift one that refuses them'; M4: 3 failed incl. the tenantGovernance caller-level one; M9: 1 failed 'a notice given AFTER the deadline...'; M11: 3 failed; each restored and re-run green, 94 total). Windows (incl. the past-midnight shape), freezes with strictest-wins, and the notice REQUIREMENT are real, pure, clock-injected and well-tested. REFUTED on step 3 — three of the six clauses the requirement names have no caller. `grep -rn` across the whole repo (excluding node_modules) for `cancel(`, `supersede`, `afterActionDebt`, `AFTER_ACTION_DUE_HOURS` finds hits ONLY in windows.ts itself and in e2e/change-governance-logic.spec.ts. governance.ts imports only `notificationReadiness` and `scheduleVerdict` (l.15-20); GovernancePanel.tsx renders only scheduleLines + move.notice.detail + blast lines. So cancellation, supersession and after-action review are library functions no surface imports. Emergency change is equally unreached: governance.ts:246 and :256 hardcode `emergency: null`, so EMERGENCY_OVERRIDE and afterActionReviewOwed can never occur on the page, and there is no surface for an operator to declare one. Maintenance notification is reached but constant: governance.ts:238-249 builds a ScheduledChange literal with no `notifications` field and nothing in the repo persists a ScheduledChange, so notificationReadiness always returns ready:false 'none is recorded' — the code's own comment (governance.ts:81-88) concedes 'Always "none recorded" today'. The claim's 'Rendered on the tenant page for every permitted lifecycle move' is true only of the schedule verdict and the notice line. Two of six clauses shipped and reached; four are not.
  - Code: `apps/system-studio/src/lib/change/windows.ts` — `MaintenanceWindow`, `FreezePeriod`, `ChangeCalendar`, `UNSCHEDULED_CLASSES`, `NOTICE_HOURS`, `windowContains`, `nextWindowOpening`, `freezeFor`, `noticeFor`, `scheduleVerdict`, `scheduleLines`, `NotificationRecord`, `notificationReadiness`, `ScheduledChange`, `cancel`, `supersede`, `afterActionDebt`, `AFTER_ACTION_DUE_HOURS`. `apps/system-studio/src/lib/change/calendar.ts` — `changeCalendar`, `parseCalendar`, `CalendarSource`.
  - Caller: `apps/system-studio/src/app/tenants/[slug]/page.tsx:642` calls `changeCalendar()` and `:643` `tenantGovernance(...)`; `governance.ts:223` calls `scheduleVerdict` and `:210` `notificationReadiness` once per permitted move; `src/components/GovernancePanel.tsx:50` renders `scheduleLines(move.schedule)` and `:56` `move.notice.detail`; `page.tsx:1627` mounts the panel.
  - Not the cutover freeze, and the distinction is in the header: `packages/provisioning/src/cutover-freeze.mjs` already owns the word and answers which SYSTEM may write which object during a migration. This answers whether a change may run at a given minute. Two vocabularies on purpose — one word for both sends an operator to the wrong remedy.
  - The calendar is READ, never compiled: `CHANGE_CALENDAR` (JSON), with no built-in default, because windows and freezes are estate facts and `tests/security/no-hardcoded-estate.test.mjs` exists for facts of that shape. ABSENT, DECLARED and MALFORMED are three distinct states and MALFORMED does not degrade to ABSENT — an operator who wrote a calendar and had it silently ignored is worse off than one who wrote none.
  - Default deny in the direction that matters: a window-bound class with NO window declared is `OUTSIDE_WINDOW`, not `IN_WINDOW`. An empty calendar means nobody decided when this may run.
  - A bug this work's own spec found and fixed: `freezeFor` originally returned the FIRST matching freeze, so a broad C4–C7 freeze that admits emergencies HID a narrow C7 audit lock that does not — an operator with an emergency declaration would have been told they could proceed through a lock nothing can lift. It now returns the strictest (refuses-emergencies first, then longest-running). First-match order is an array's order; a freeze is a rule.
  - Windows that run past midnight work: `endMinuteUtc` may exceed 1440, and `windowContains` matches both the Saturday evening and the Sunday morning of a 23:00–03:00 window — the shape a naive `start < end` comparison silently never matches.
  - Maintenance notifications are REQUIRED, RECORDED and CHECKED. `NOTICE_HOURS` is a band table (C4/C5 = 24h, C6/C7 = 72h, C1–C3 = none), `noticeFor` computes the deadline and whether `now` is already past it, and `notificationReadiness` decides whether a notice was actually given — a record whose `sentAt` is after `dueBy` does NOT count, because a notice sent an hour before a purge is a log entry and not a warning. On the tenant page every window-bound move currently reads "72h notice was required by <dueBy> and none is recorded. This console does not send notices; record the one that was sent."
  - Cancellation refuses a change that has already started (a "cancelled" written over a change that ran is a record contradicting the estate) and a reason shorter than `MIN_OVERRIDE_REASON`, imported from `@tenure/provisioning` rather than re-chosen. Supersession refuses across targets (which would drop a change nobody cancelled), refuses a replacement that is not later, and refuses self-supersession.
  - Emergency is a permission, not a bypass: it lifts a closed window and a freeze that admits emergencies, it does NOT lift one that refuses them, and it always sets `afterActionReviewOwed`. `afterActionDebt` lists emergency changes that ran with no review, keeping "owed" and "overdue" as separate facts.
  - Tests: `apps/system-studio/e2e/change-governance-logic.spec.ts` — 46 tests, browser-free; `apps/system-studio/e2e/tenant-governance-logic.spec.ts` — 21 tests including the six calendar-parse arms and the caller-level notice assertions.
  - Evidence: `cd apps/system-studio && ../../node_modules/.bin/playwright test e2e/change-governance-logic.spec.ts e2e/portability-logic.spec.ts e2e/tenant-governance-logic.spec.ts --reporter=line` → `94 passed (8.7s)`. `npx tsc --noEmit` → 0 errors.
  - Mutation proven, four times, one literal each: strictest-freeze sort inverted → **1 red** (`a declared emergency does NOT lift one that refuses them`); `permitted: false` → `true` on OUTSIDE_WINDOW → **3 red** (incl. `an empty calendar is not permission`); the notice deadline compared against `scheduledFor` instead of `dueBy` → **1 red** (`a notice given AFTER the deadline is a log entry, not a warning`); `ready: false` → `true` for a change with no notice → **3 red** (incl. the caller-level `a move that owes a maintenance notice cannot be called notified`). All restored, all re-run green.
  - Honest limit, stated rather than glossed: **nothing SENDS a notice.** There is no notification transport in `apps/system-studio` — every `@aws-sdk/client-sesv2` use in this app is a reader. What this console does is require the notice, hold the record of one, and refuse to call a change notified without it; a `sentAt` written by the code that was supposed to send it would be a claim with nothing behind it. Second limit: `ScheduledChange`, `cancel`, `supersede` and `afterActionDebt` operate on a change record that **nothing persists yet** — the registry has no `CHANGE#<id>` item. They are exported, tested and mutation-proven, and `notificationReadiness` is the only one of the four with a production caller today. Persisting the record is STUDIO-060-009, which is NEEDS_SCHEMA.

## STUDIO-040-008 — a tenant's desired state leaves the building, checked against its own rule on the way out and read back on the way in
...
- [ ] **STUDIO-040-008** — "Make configuration import/export portable without exporting platform secrets, AWS credentials, other tenants, or internal exploit-sensitive policy details."
  - Status: FAIL
  - Overturned on review: Mutations reproduce (M5: 1 failed 'a confidential value is withheld and a public one is carried'; M6: 6 failed, all the leak-refusal tests; M10: 1 failed 'the bundle it shows is one this engine reads back'; each restored, 94 green). The leak scanner is genuinely real for AWS keys/ARNs/account ids/private keys/foreign slugs, and the self-check over finished output is not decorative. REFUTED on step 1, on the clause the claim does not address. The requirement forbids exporting 'internal exploit-sensitive policy details'; bundle.ts:47 `PORTABLE_SENSITIVITIES = ['public','internal']` deliberately lets every `internal` value travel. packages/platform-config/src/definitions.ts:237-260 defines `platform.payments.approvalThresholds` (key at l.235) with `sensitivity: 'internal'`; it is in PLATFORM_DEFINITIONS (l.354) -> REGISTRY, and page.tsx:655 passes `REGISTRY.all().map(d => ({key, sensitivity}))` straight into exportBundle, whose only filter is `PORTABLE_SENSITIVITIES.includes(sensitivity)` (bundle.ts:236). A tenant's approval-authority ceilings are precisely the internal policy detail an attacker structures transactions beneath, and the portable bundle carries them. e2e/portability-logic.spec.ts:102 actively ASSERTS this ('every value that DOES travel is declared public or internal in the real registry'), so the test locks the behaviour the requirement forbids rather than catching it. Same applies to the `internal` flag definitions in packages/platform-config/src/flags.ts:188,209,263. Second, independent gap: there is no import/export. `ls src/app/api/` = auth, aws, export (the estate export, which bundle.ts:16-21 explicitly disclaims); no route emits a bundle and nothing accepts one. `importBundle` is called in exactly one place, governance.ts:283, on `JSON.parse(JSON.stringify(bundle))` — the console's own freshly-built object. bundleLines() renders three summary lines; the JSON never leaves. A round-trip self-check is evidence about a format, not an import/export the requirement can be 'made portable'.
  - Code: `apps/system-studio/src/lib/portability/bundle.ts` — `BUNDLE_VERSION`, `PORTABLE_SENSITIVITIES`, `PortableBundle`, `Withholding`, `Leak`/`LeakKind`, `bundleLeaks`, `PortabilityRefused`, `exportBundle`, `importBundle`, `bundleLines`.
  - Caller: `apps/system-studio/src/app/tenants/[slug]/governance.ts:240` calls `exportBundle`, `:256` calls `importBundle` on the serialised result; `src/components/GovernancePanel.tsx:83` renders `bundleLines(...)` and `:90` the read-back verdict; `apps/system-studio/src/app/tenants/[slug]/page.tsx:643` calls `tenantGovernance` and `:1627` mounts the panel.
  - Not the estate export, and the header says so: `src/app/api/export/route.ts` exports inventory/drift/posture for an operator's own analysis and names AWS accounts, ARNs and security groups ON PURPOSE, because the operator reading it already has them. This is the artifact that LEAVES. Two artifacts, two rules; collapsing them is how an estate inventory ends up in a customer's inbox.
  - Redaction is driven by a field that already exists: `ConfigDefinition.sensitivity` in `@tenure/configuration`. `public` and `internal` travel; `confidential` and `secret` do not; and a key with NO definition in the registry does not travel either — an unreviewable key is not a safe key. No second "exportable" flag was invented, because two lists that can disagree eventually do and the wrong one is the one nobody looks at.
  - Secret references become slot NAMES with the pointer removed (`secretsmanager:tenure/simon/stripe` → `"stripe"`, plus a `withheld` entry saying why): an importer needs to know a slot exists, not where this platform's vault is. `initialAdminEmail` is withheld as personal data. Every removal is recorded in `withheld` with its reason, so the bundle says what it is NOT carrying.
  - The self-check is the point: `exportBundle` builds the bundle, runs `bundleLeaks` over what it BUILT, and throws `PortabilityRefused` rather than returning a partly-sanitised artifact. A redaction applied only on the way in stops covering the next field somebody adds; a check over the finished object covers fields nobody thought about. Seven leak kinds: AWS access key id, ARN, twelve-digit account id, secret-store reference, private-key block, email address, and another tenant's slug. It throws rather than returning `{bundle, leaks}` because a caller handed both writes the file first and reads the second field never.
  - `bundleLeaks` skips the `withheld` array by construction, because those reasons NAME the shapes they removed ("a pointer into this platform's secret store") and a scanner that read them would report the redaction as the leak. Asserted directly.
  - Import is not just a parser: `importBundle` refuses an unknown `bundleVersion`, a missing required field, a non-object, and — the clause that makes it more than a parser — a bundle that carries anything the export rule forbids, so a foreign bundle naming another estate's ARNs is refused rather than absorbed into this registry.
  - Portability is a ROUND TRIP: the tenant page serialises the bundle it is about to show and puts it back through `importBundle`, then renders either "Read back by this engine's own importer: this bundle is portable, not just exported" or the problems. Serialised and re-parsed rather than handed over as an object, because what leaves is JSON and an object that satisfies the type is not evidence that its serialisation does.
  - Tests: `apps/system-studio/e2e/portability-logic.spec.ts` — 27 tests, browser-free. The real `REGISTRY.all()` definitions are used for the property that must hold over what ships ("every value that DOES travel is declared public or internal in the real registry"); `confidential` keys are supplied by hand, and the file says why in as many words: the platform registry declares none today, so a test using only the real registry would pass by never exercising the branch that withholds them.
  - Evidence: `cd apps/system-studio && ../../node_modules/.bin/playwright test e2e/change-governance-logic.spec.ts e2e/portability-logic.spec.ts e2e/tenant-governance-logic.spec.ts --reporter=line` → `94 passed (8.7s)`. `npx tsc --noEmit` → 0 errors. `node --test tests/security/no-personal-data.test.mjs tests/security/no-hardcoded-estate.test.mjs tests/architecture/no-fixture-tenants-on-operator-surfaces.test.mjs` → `# pass 10 # fail 0`.
  - Mutation proven three times: `PORTABLE_SENSITIVITIES` widened to include `confidential` → **1 red**; the leak guard in `exportBundle` short-circuited to `if (false)` → **6 red**, all five leak kinds plus the refusal-carries-its-leaks test; the round-trip result replaced with a constant in the caller → **1 red**. All restored, all re-run green.
  - Honest limit: there is no file DOWNLOAD and no file UPLOAD. The bundle is rendered on the tenant page and the importer's only production caller is the round-trip check. Offering the bundle as a file and accepting one back is a new API route, and a new route makes `docs/architecture/entry-points.md` stale — a shared generated doc I deliberately did not regenerate while eleven agents are mid-flight. The portability RULE and both directions of the transform are shipped and proven; the transport is not.

## STUDIO-040-009 — a clone copies what a tenant IS and nothing it HOLDS, and still nobody can create one

- [ ] **STUDIO-040-009** — "Implement tenant clone as a sanitized blueprint/manifest copy, never a production-data or credential copy."
  - Status: FAIL
  - Reason, precisely: the sanitisation semantics are implemented, wired and mutation-proven, and **no path creates the clone**. `cloneTenant` returns a manifest; the only writer that can turn a manifest into a tenant is `composeTenant` in `apps/system-studio/src/app/tenants/actions.ts`, which is a server action driven by the form at `src/app/tenants/new/page.tsx`, and nothing hands the clone manifest to it. An operator can READ what a clone would and would not carry; they cannot make one. The verb in the requirement is "implement tenant clone", and a preview is not a clone.
  - What landed, and is proven:
    - Code: `apps/system-studio/src/lib/portability/clone.ts` — `cloneTenant`, `cloneLines`, types `CloneRequest`, `CloneDrop`, `CloneProblem`, `CloneOutcome`, `CloneContext`.
    - Caller: `apps/system-studio/src/app/tenants/[slug]/governance.ts:266` calls `cloneTenant` against the sanitized bundle; `src/components/GovernancePanel.tsx:99` renders `cloneLines(...)`; `apps/system-studio/src/app/tenants/[slug]/page.tsx:1627` mounts the panel.
    - The source is the PORTABLE BUNDLE, not the registry record, and that is the design decision that carries the requirement. A clone built from `TenantRecord` would have the secret refs, the placement, the deployment artifact and the evidence rows in scope — every one a field somebody has to remember not to copy, and the failure mode is a clone that carries a secret pointer because a field was added after the clone code was written. Building from the bundle means the set of things a clone COULD accidentally copy is exactly the set `exportBundle` lets out: one rule, one place, with a self-check over its own output.
    - `cloneTenant` runs `bundleLeaks` again anyway — a bundle can arrive from a file — and REFUSES rather than sanitising. Proven: an access key id planted in a bundle produces `leak:aws-access-key` and no clone.
    - `secretRefs: {}` always, with one `CloneDrop` per slot the source needed, so the operator sees each binding they now owe. `initialAdminEmail` comes from the request; the source's administrator is a person who did not ask to administer this.
    - A configuration value whose text NAMES the source tenant is dropped with its reason — a hostname or bucket prefix carried into a clone points the new system at the old one.
    - Every omission is returned rather than being silently absent: `data`, `placement`, `domains`, `initialAdminEmail`, and each secret slot, each with a sentence. A clone that looks complete and is not is worse than one that lists what is missing.
    - Refusals: reusing the source's slug, taking a slug this installation already uses, and a bundle written at a manifest version newer than this engine reads (fields it does not know about would be dropped without anyone deciding to drop them).
    - Tests: `apps/system-studio/e2e/portability-logic.spec.ts` (27 tests) — nine of them on clone; `apps/system-studio/e2e/tenant-governance-logic.spec.ts` (21 tests) — the caller-level preview.
    - Evidence: `cd apps/system-studio && ../../node_modules/.bin/playwright test e2e/change-governance-logic.spec.ts e2e/portability-logic.spec.ts e2e/tenant-governance-logic.spec.ts --reporter=line` → `94 passed (8.7s)`. `npx tsc --noEmit` → 0 errors.
    - Mutation proven twice: the source-naming drop short-circuited to `if (false)` → **1 red** (`drops a configuration value that names the source tenant`); and in the CALLER, `initialAdminEmail: CLONE_PREVIEW_ADMIN` replaced with the source's own address → **1 red** (`the clone preview carries no administrator from the source`). The second is mutated in the caller deliberately: a library-only test would have stayed green while the page leaked a real person's address. Both restored, both re-run green.
  - What would close it: a `?cloneFrom=<slug>` path into `src/app/tenants/new/page.tsx` that prefills the compose form from `cloneTenant`'s manifest and lets `composeTenant` — which already runs `validateManifest` against the real blueprint and module catalogues — do the writing. That is a change to a shared route and a rendered-page verification, i.e. a build. It is NOT blocked by anything external, and it needs no schema.
