...
- [x] **PAY-060-001** — "Implement canonical payment order/attempt state machines independent of Stripe object state."
  - Status: PASS
  - What was open: nothing in the tree modelled a payment order. The provider-event inbox recorded a receipt and stopped, so the only vocabulary the platform had for the state of a payment was Stripe's own event type string. "Independent of Stripe object state" had nothing to be independent OF.
  - Code: `packages/payments/src/payment-order-state.ts` — `PAYMENT_ORDER_STATES` (Bible §8's thirteen, in §8's order), `PAYMENT_CONTROL_STATES` (§8's thirteen control/failure states, verbatim), `PAYMENT_ATTEMPT_STATES` (a SECOND machine: it has `ABANDONED`, which an order is not, and lacks `RECONCILED`, which an attempt is not), `ORDER_TRANSITIONS`, `ATTEMPT_TRANSITIONS`, `EVIDENCE_SOURCES`, `NON_SETTLEMENT_EVIDENCE`, `SETTLEMENT_STATES`, `advanceOrder`, `advanceAttempt`, `observeProviderState`, `canonicalProviderReadings`, `ACCOUNT_SCOPED_EVENTS`, `isTerminalOrderState`. Independence is enforced three ways, not asserted: the state set contains `SETTLEMENT_PENDING`, `RECONCILED` and `CLOSED`, which no provider object has; `observeProviderState` returns `{providerState, observed, reading, source, note}` with no `from` and no resulting order, so it cannot be mistaken for a transition; and it answers THREE ways — `CANONICAL`, `NOT_A_PAYMENT_ORDER_EVENT` (an account event is legible and simply is not about an order) and `UNRECOGNISED` (`UNKNOWN_PROVIDER_STATE`) — rather than collapsing "not about an order" into "we could not look".
  - Caller: `apps/web/src/app/api/payments/provider-events/route.ts` — imports `observeProviderState` (line 7 of the import block) and calls it after dedupe, returning `observed` / `reading` / `applied: false` on both the duplicate and the new-event acknowledgement. Exported from the package root at `packages/payments/src/index.ts` (lines 326-351).
  - Reached, with one limit stated: the route is a real production surface and it consults the reader. `advanceOrder` and `advanceAttempt` are so far exercised only by tests — no surface applies a transition yet, because applying one needs somewhere to keep the order's current state.
  - Not done, and deliberately: the observation is RETURNED, not persisted. `ProviderEventReceipt` has no column for it and this change makes no schema change. Persisting it needs a migration adding an observed-state column; the route comment says so in those words.
  - Tests: `packages/payments/src/payment-order-state.test.ts` (22 tests) and `apps/web/src/app/api/payments/provider-events/route.test.ts` (5 tests, over a real HMAC with signature verification, version check, parser and dedupe all unmocked). Plus `tests/architecture/pay-canonical-payment-and-movement-commands.test.mjs` tests 1–4, of which 2 and 3 parse §8's lifecycle line and control-state line OUT OF THE BIBLE and compare them with the arrays parsed out of the TypeScript, so a drift in either direction fails.
  - The two assertions worth naming: `route.test.ts` proves a `payout.paid` event reads as `SETTLED` while nothing is advanced, and `payment-order-state.test.ts` proves that a `payment_intent.succeeded` observation applied to a `DRAFT` order is refused `order-transition-forbidden` — which is the whole requirement.
  - Mutation 1: `if (ACCOUNT_SCOPED_EVENTS.includes(providerState)) {` -> `if (false) {` -> `Tests: 2 failed, 25 passed, 27 total`; the route test reported `Expected: "NOT_A_PAYMENT_ORDER_EVENT" / Received: "UNRECOGNISED"`. Restored -> `Tests: 27 passed, 27 total`.
  - Mutation 2 (the Bible half): `"DRAFT"` -> `"DRAFTED"` -> `not ok 2 - PAY-060-001 — the lifecycle is Bible §8's lifecycle, in §8's order`, `# pass 7 # fail 1`. Restored -> `# pass 8 # fail 0`.
  - Mutation 3 (refuter, the §8 settlement rule): the `SETTLEMENT_STATES && NON_SETTLEMENT_EVIDENCE` guard in `advanceOrder` -> `if (false)` -> `Tests: 4 failed, 23 passed, 27 total`, all four "refuses SETTLED on CLIENT_REDIRECT / SYNCHRONOUS_API_RESPONSE / EMAIL / ELAPSED_TIME".
  - Mutation 4 (refuter, the attempt half neither original mutation touched): `if (!allowed.includes(input.to))` in `advanceAttempt` -> `if (false)` -> `Tests: 1 failed, 21 passed, 22 total`, killing "makes a retry a new attempt rather than a step out of FAILED".
  - Evidence: `cd apps/web && npx jest --ci payment-order-state provider-events` -> `Test Suites: 2 passed, 2 total / Tests: 27 passed, 27 total`. `node --test tests/architecture/pay-canonical-payment-and-movement-commands.test.mjs` -> `# tests 8 # pass 8 # fail 0`.
...
- [x] **PAY-080-001** — "Implement explicit internal-allocation, internal-transfer, intercompany and external-movement command types."
  - Status: PASS
  - What was open: `classifyRequest` (PAY-180-006) answered whether a movement may proceed, over a nine-value `MoneyMovementKind`. Nothing answered what it IS. A club reimbursement, a posting between two universities and an instruction to pay an outside vendor all reached the same `db.ledgerEntry.create`, and the row they wrote was indistinguishable afterwards.
  - Code: `packages/payments/src/movement-commands.ts` — `MOVEMENT_COMMAND_TYPES` (`INTERNAL_ALLOCATION`, `INTERNAL_LEDGER_TRANSFER`, `INTERCOMPANY_TRANSFER`, `EXTERNAL_PROVIDER_MOVEMENT`, in §10's order), `classifyMovementCommand`, `MovementCommandError`, `assertProviderCallPermitted`, types `MovementCommandFacts` / `MovementCommandDecision`. It IMPORTS `MONEY_MOVEMENT_KINDS` from `refusal.ts` rather than restating it, so a kind added there cannot silently acquire a §10 type here — the test pins `MONEY_MOVEMENT_KINDS.length` at 9 and asserts all nine classify.
  - Fails closed, and the undecidable cases are FIVE different codes rather than one: `movement-command-source-unreadable` (blank legal entity), `movement-command-unknown-kind`, `movement-command-memo-posts-journal` (§10's memo is DEFINED by making no accounting posting, so a memo that posts is not a memo), `movement-command-ledger-transfer-posts-nothing`, `movement-command-internal-names-external-beneficiary`. Every undecided decision carries `commandType: null`, `payoutCommand: null` and `providerCallPermitted: false` — a test asserts that over all five. An unreadable fact is reported as UNKNOWN, never defaulted to a type.
  - Caller: `apps/web/src/app/(app)/approvals/actions.ts` — imports `classifyMovementCommand` (line 20) and calls it at line 753 in the reimbursement-posting block beside `classifyRequest`, with `postsJournal: true` because what follows a clearance there is `buildJournal` and two `LedgerEntry` rows. Two effects: (a) an ALLOWED movement whose §10 type could not be decided is refused with a `Payments.COMMAND_UNCLASSIFIED` DENY row and never posts; (b) every Payments refusal row now carries `commandType`, `commandCode` and `payoutCommand` in its metadata, and every successful posting carries `commandType` / `commandCode` / `requiresIntercompanyPolicy` in the decision step's `policySnapshot.movement`.
  - Reachability, precisely: THREE of the four types are reachable from that caller. `INTERNAL_ALLOCATION` is NOT — the caller passes `postsJournal: true` unconditionally, so a `memo` on this path is always refused rather than classified. That is correct behaviour for a path that writes a journal, but it means the fourth type is produced only by the package-level classifier, not by this surface. Each of the four has a test on the ACTION: a club reimbursement POSTS as `INTERNAL_LEDGER_TRANSFER` (asserted on `policySnapshot.movement`); a `memo` is refused `movement-command-memo-posts-journal`; a cross-institution allocation and a payout are refused by `classifyRequest` BEFORE any posting and carry `INTERCOMPANY_TRANSFER` / `EXTERNAL_PROVIDER_MOVEMENT` on the DENY audit row's metadata — recorded on a refusal, not on a journal.
  - `assertProviderCallPermitted` is exported but has no production caller; there is no provider client in this tree (`tests/architecture/payments-port-is-the-only-door.test.mjs` fails the build if one appears). It guards a door a future adapter must come through, and is PAY-080-002's evidence, not this row's.
  - Tests: `packages/payments/src/movement-commands.test.ts` (23 tests) and `apps/web/src/app/(app)/approvals/movement-command-type.test.ts` (5 tests, driving the real `actOnApproval` with only the database, session, scope, RBAC and notifications mocked). Both are pure logic — no dates, locale, randomness or host paths. `tests/architecture/pay-canonical-payment-and-movement-commands.test.mjs` test 5 parses §10's four bolded bullets out of the Bible and DERIVES the expected constant names from them, so a fifth bullet in §10 fails rather than being ignored; test 6 asserts the action still calls the classifier and still refuses `!command.decided`.
  - Mutation 1: `if (facts.postsJournal) {` -> `if (false) {` -> `Tests: 3 failed, 25 passed, 28 total`, red in both the package suite and at the action. Restored -> `Tests: 28 passed, 28 total`.
  - Mutation 2 (the wiring, not the rule): removed `commandType` / `commandCode` / `requiresIntercompanyPolicy` from the `movementEvidence` block in `actions.ts` -> `Tests: 1 failed, 4 passed, 5 total`. Restored -> `Tests: 5 passed, 5 total`.
  - Evidence: `cd apps/web && npx jest --ci movement-commands movement-command-type` -> 28/28; the wider run `npx jest --ci payment-order-state payout-commands movement-commands provider-events movement-command-type money-movement payment-movement-gate` -> `Test Suites: 7 passed, 7 total / Tests: 92 passed, 92 total`. `node --test` over the five payments architecture files -> `# tests 32 # pass 32 # fail 0`. All independently reproduced by the refuter.
...
- [x] **PAY-090-001** — "Implement distinct semantic commands and state machines for settlement payout, transfer, outbound payment, refund and disbursement."
  - Status: PASS
  - What was open: Bible §11 opens "Do not use one generic 'payout' verb" and lists ten distinct movements. The tree had one word — `payout` — as a value of `MoneyMovementKind`, and no state machine for any of them.
  - Code: `packages/payments/src/payout-commands.ts` — `PAYOUT_COMMANDS` (`SETTLEMENT_PAYOUT`, `BALANCE_TRANSFER`, `OUTBOUND_PAYMENT`, `REFUND`, `DISBURSEMENT`), `PAYOUT_COMMAND_MACHINES` (per command: `summary`, `initial`, `transitions`, `counterparty`, `bankRail`), `transitionPayoutCommand`, `statesOf`, `isTerminalPayoutState`.
  - The machines are distinct rather than five labels on one table, and it is asserted three ways: no two commands share a state set (7 / 6 / 9 / 5 / 8 states); no two share a counterparty (`CONNECTED_ACCOUNT_OWN_BANK`, `PLATFORM_OR_CONNECTED_BALANCE`, `THIRD_PARTY_BENEFICIARY`, `ORIGINAL_PAYMENT_METHOD`, `INTERNAL_PERSON`); and `RETURNED` exists on exactly the three that cross a bank rail, so a `REFUND` asked to go `RETURNED` is refused `payout-state-not-of-command` with a message naming the three commands that DO own that state. `BENEFICIARY_VERIFIED` exists only on `OUTBOUND_PAYMENT` and its `APPROVED` is unreachable without passing through it — which is the one property that makes an outbound payment different from a payout.
  - The generic verb is refused by name: `transitionPayoutCommand({command: "payout", ...})` returns `payout-command-unknown` with "there is no generic payout verb, so there is no generic machine to fall back to".
  - Caller: `packages/payments/src/movement-commands.ts` maps each leaving `MoneyMovementKind` to its §11 verb (`PAYOUT_COMMAND_BY_KIND`) and returns it as `payoutCommand`; `apps/web/src/app/(app)/approvals/actions.ts` records it on the `Payments.REFUSED` / `Payments.ESCALATE` audit row. `charge` and `payroll` return `null` and say "case by case" rather than being forced into one of the five — §11 names no single verb for either.
  - Honest scope: nothing can execute any of these. `classifyRequest` refuses every leaving kind outright, and `@tenure/payments` carries no write verb (`tests/architecture/payments-port-is-the-only-door.test.mjs` still green). This is the taxonomy and the legal sequences, not the Payout Service — `docs/payments/bounded-contexts.md` §1 now says exactly that for context 5.
  - Tests: `packages/payments/src/payout-commands.test.ts` (17 tests, including one walk per command through its own path to a terminal state). `tests/architecture/pay-canonical-payment-and-movement-commands.test.mjs` tests 7–8 parse the five names OUT OF THE PAY-090-001 line in the Bible and derive the expected constants, and recompute the per-command state sets from the source to assert the fingerprints are five distinct values.
  - Mutation 1: `payout: "SETTLEMENT_PAYOUT"` -> `payout: "BALANCE_TRANSFER"` -> `Tests: 2 failed, 26 passed, 28 total`, red in the package suite AND at the action, which is the proof it is reached from production. Restored -> `Tests: 28 passed, 28 total`.
  - Mutation 2: added `"APPROVED"` to `OUTBOUND_PAYMENT.transitions.REQUESTED` -> `Tests: 1 failed, 16 passed, 17 total` on `puts BENEFICIARY_VERIFIED on the one command with a third party to verify`. Restored -> `Tests: 17 passed, 17 total`.
  - Evidence: `cd apps/web && npx jest --ci payout-commands` -> `Tests: 17 passed, 17 total`. `node --test tests/architecture/pay-canonical-payment-and-movement-commands.test.mjs tests/architecture/pay-bounded-contexts.test.mjs tests/architecture/payments-port-is-the-only-door.test.mjs tests/architecture/pay-no-module-bypasses-the-payment-layer.test.mjs tests/architecture/pay-authority-boundary-and-adrs.test.mjs` -> `# tests 32 # pass 32 # fail 0`.
...
- [x] **PAY-180-004** — "Redact/tokenize financial identifiers in prompts, logs, traces and model outputs."
  - Status: PASS
  - What was open in the ledger: the row only; the code shipped in an earlier wave unrecorded. What I added is the verification the sentence demands — the sentence names FOUR destinations, and a single test run proving "redaction happens" would not distinguish four wired paths from one wired path and three assumptions. So one mutation per path.
  - Code: `packages/payments/src/financial-identifiers.ts` decides WHAT an identifier is (Luhn, ISO 13616 IBAN mod-97, ABA check plus assigned Federal Reserve prefix, provider-object prefixes) and how much a purpose may see. `apps/web/src/lib/payments/financial-redaction.ts` is the adapter supplying the two facts a package cannot know: which tenant is in scope (`currentScope()`) and whether this deployment holds `PAYMENTS_TOKENIZATION_KEY`. Exports `tokenizationKey`, `scrubForModel`, `scrubForLog`, `scrubForAudit`, `scrubStructuredForLog`, `describeScrub`, `TOKENIZATION_KEY_VAR`.
  - Why redact rather than refuse, which is the opposite of the credential answer: `ai.ts` REFUSES a prompt carrying a `whsec_…` because that is a leak with no upside. A card number arrives in the ordinary course of the product — a treasurer pastes a receipt — and refusing every question whose sources contain one would take the assistant away from exactly the people asking about money. The value is removed and a TOKEN left, so the model can still reason about "the same card twice" without the value crossing the boundary. Without a key the value is still removed and the replacement SAYS it could not be tokenized (`tokenRefusal: "no-key"`), because "we removed this and can tell you it is the same card as the other one" and "we removed this" are different answers.
  - Callers, one per named destination:
    - PROMPTS — `apps/web/src/lib/ai.ts:191-192`, `scrubForModel(system)` and `scrubForModel(user)`, on the one function in this application that posts text to a model vendor (`api.anthropic.com` is fetched at `ai.ts:232` and nowhere else).
    - MODEL OUTPUTS — `apps/web/src/lib/ai.ts:276`, `raw === null ? null : scrubForModel(raw).text`.
    - LOGS — `apps/web/src/lib/ai.ts:297,308`, `scrubForLog(safeLogText(...)).text` on the provider error body and on the thrown error. These are the only two `console.*` calls on this surface that can carry prompt-derived text.
    - TRACES — `apps/web/src/lib/audit-record.ts:513,517,518,522`, `scrubForAudit` over `reason`, `change.before`, `change.after` and `metadata`. `recordAuditEvent` is this application's trace path — there is no OpenTelemetry, no logger module and no request-logging middleware in `apps/web`, so the chained audit row is the only durable per-request trace — and it is written unconditionally, ALLOW and DENY alike, on every Relay request at `apps/web/src/app/api/ai/chat/route.ts:504` (imported at `chat/route.ts:23`). It is also the destination where a leak is permanent — the table is append-only with `ON DELETE RESTRICT` — which is why redaction runs before the change block is digested.
  - Tests: `apps/web/src/lib/payments/prompt-redaction.test.ts` — 14 pass, asserted on the PRODUCER: the real `aiComplete` with `global.fetch` stubbed, reading what would actually have gone on the wire. Nine on the prompt (card removed; system prompt scrubbed too; it still ANSWERS rather than refusing, which is the difference from a credential; the same card gets the same token across two calls; two tenants get DIFFERENT tokens for the same card; no key still removes and says so; outside a tenant it refuses to token-scope rather than tokenizing into a shared namespace; a clean prompt is byte-for-byte unchanged; the log line carries counts and kinds and no value). Two on the model's own output. One on the log sink. Two on `describeScrub`. `apps/web/src/lib/payments/audit-evidence-wired.test.ts` — the six-test `what a financial identifier does when it reaches the audit trail` describe covers the trace path, including that the change block's digest still matches what is STORED after redaction. Neither spec depends on this host: the vendor is a stubbed `fetch`, the key and both env vars are set and restored inside the file, and every assertion is on a literal or a `tk_<kind>_[0-9a-f]{24}` shape.
  - Mutation proof — four, one per path, each restored byte-identical before the next:
    - prompt: `scrubForModel(user)` -> `{ text: user, findings: [] }` -> `Tests: 6 failed, 8 passed, 14 total`; restored -> 14 pass.
    - model output: `raw === null ? null : scrubForModel(raw).text` -> `raw` -> `Tests: 1 failed, 13 passed, 14 total`; restored -> 14 pass.
    - log: `scrubForLog(safeLogText(detail.slice(0, 500))).text` -> `safeLogText(detail.slice(0, 500))` -> `Tests: 1 failed, 13 passed, 14 total`; restored -> 14 pass.
    - trace: `scrubForAudit(input.reason, input.institutionId)` -> `input.reason` -> `Tests: 3 failed, 8 passed, 11 total`; restored -> 11 pass.
  - Evidence: `cd apps/web && npx jest --ci --testPathPattern "prompt-redaction"` -> `Tests: 14 passed, 14 total`; `... "audit-evidence-wired"` -> `Tests: 11 passed, 11 total`.
  - Honest limits, two: (1) `scrubForLog` has exactly one importer. Every other `console.error` in the application is unscrubbed. Bible §20 scopes this requirement to the Relay surface — the general log-redaction requirement is PAY-170-008, a separate row — so that is out of this row's sentence, but it is where the next leak will be. (2) `scrubStructuredForLog` is exported and has no caller; it is available for a structured trace payload, not yet on a shipped path.
  - Refuter (independent): authority sentence read at Bible line 685; all four mutations re-applied and re-run individually, all four failed with the exact counts and test names claimed, all four restored `diff -q` clean; every cited path, exported name and line number re-checked. One correction applied to this row — the original cited the per-request audit write as `chat/route.ts:464`, which is the `budgetVerdict` line; the `recordAuditEvent` call is at `chat/route.ts:504`.
