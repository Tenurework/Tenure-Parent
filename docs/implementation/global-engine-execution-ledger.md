...
- [x] **GE-092-004** — "Implement reranking, deduplication, diversity, freshness, contradiction detection, citation completeness, and context budgeting."
  - Status: PASS
  - What was there: `/api/ai/chat` did `rankDocs(corpus, question, 24)` then `biasToScope(ranked, askScope).slice(0, 6)`. A relevance sort and a fixed count. Six of the seven things this requirement names were absent, and each absence has a specific wrong answer behind it — the same record offered twice reads to a model as corroboration; six rows from the one club whose documents happen to use the query's words most is an answer the person is never told is narrow; six 1000-character bodies and six 40-character ones cost the tenant thirty times different amounts for the same "six sources".
  - Code: `apps/web/src/lib/relay/evidence-assembly.ts` — `assembleEvidence()`, `detectContradictions()`, `extractAssertions()`, `citationGaps()`, `citationResolves()`, `dedupeKeys()`, `normalizeText()`, `subjectOf()`, `estimateTokens()`, plus `CHARS_PER_TOKEN`, `DEFAULT_EVIDENCE_TOKEN_BUDGET`, `DEFAULT_MAX_SOURCES`, `DIVERSITY_LAMBDA`, `FACT_KEYS` and the `EvidenceSource` / `EvidenceAssembly` / `Contradiction` / `DropReason` types.
  - Caller: `apps/web/src/app/api/ai/chat/route.ts:40` imports it; `:374` calls `assembleEvidence(biasToScope(ranked, askScope), {...})` and `:389` takes `scored` from its selection. The route passes its own `costOf` (`:379`) so the budget is charged against what `modelSourceFor` will actually send — a REFERENCE_ONLY card costs its heading, not the body that is never sent. The drops reach the person at `:832` (`evidence.droppedForBudget`, `droppedAsDuplicate`, `tokensUsed`, `tokenBudget`, `staleCount`).
  - One deliberate consequence outside the new file: `biasToScope` (`route.ts:171`) now writes its weighting ONTO the doc instead of sorting and discarding it. It was correct to discard while the only consumer was `.slice(0, 6)` — order was the whole answer — and a scope bias that survived only as array position would have been silently dropped by the selection that replaced the slice.
  - Design decisions worth recording, because each was a fork:
    - Diversity is a multiplicative rank penalty `score / (1 + 0.25·repeats)`, not a per-club cap. Scale-free, so the day somebody changes a weight in `search.ts` from 6 to 60 the diversity term does not become a rounding error; and it REORDERS rather than excludes, so a question genuinely about one club is still answered from that club. Both properties have tests.
    - Citation completeness DROPS only when the citation cannot name a record (`ref.externalId` / `ref.provider`). A source missing only a timestamp is still resolvable to an exact record, so it is offered and its gap is DECLARED to the model. "We could not look" and "it is current" are different answers.
    - `detectContradictions` is lexical over an allowlist of fact keys (`status`, `amount`, `due`, `venue`, …), not semantic, and the module's header says so in as many words. The shape is real rather than hypothetical: `search-data.ts` builds every approval body as `${description} status:${status.toLowerCase()}`. A caller that believed it was semantic would read "no contradictions" as "these sources agree".
    - Every ordering is total (score, then recency, then id). A comparator returning 0 leaves the selection to sort stability over whatever order the database returned, which makes the answer a property of the query plan.
  - Tests: `apps/web/src/lib/relay/evidence-assembly.test.ts` — 22 passed, 22 total. Citations in every fixture are built by the REAL producer `projectTenureRecord`, not hand-written. `apps/web/src/app/api/ai/chat/evidence-provenance.test.ts` — 17 passed, 17 total — POSTs to the route so the selector is proved at its caller.
  - Evidence: five mutations, one literal at a time, each restored:
    - `DIVERSITY_LAMBDA = 0.25` -> `0` => `> promotes a lower-scoring source from a club nothing else covers` / `Tests: 1 failed, 21 passed, 22 total`
    - `return external ? [external, text] : [text]` -> `return [text]` => `> drops the second projection of the same external record` / `Tests: 1 failed, 34 passed, 35 total`
    - `> tokenBudget` -> `> 999999` => `> drops the source that does not fit and keeps a smaller one behind it` / `Tests: 1 failed, 21 passed, 22 total`
    - `citationResolves` forced true => `> drops a source whose citation cannot name a record` / `Tests: 1 failed, 21 passed, 22 total`
    - `FACT_KEYS` `"status"` -> `"statuz"` => `Tests: 5 failed, 30 passed, 35 total`
    - the caller: `const scored = evidence.selected.map(...)` -> `biasToScope(ranked, askScope).slice(0, 6)` => `Tests: 1 failed, 12 passed, 13 total`
    - restored: `npx jest src/lib/relay src/app/api/ai` -> `Test Suites: 18 passed, 18 total / Tests: 320 passed, 320 total`
...
- [x] **GE-102-001** — "Implement `DRAFT → VALIDATING → PLANNED → AWAITING_APPROVAL → PROVISIONING → CONFIGURING → MIGRATING → VERIFYING → READY → ACTIVATING → ACTIVE` plus explicit `IDLE`, `SUSPENDING`, `SUSPENDED_LOGICAL`, `HIBERNATING`, `HIBERNATED_ZERO_RUNTIME`, `REACTIVATING`, `EXPORTING`, `OFFBOARDING`, `LEGAL_HOLD`, `PURGE_PENDING`, `PURGING`, `PURGED_ZERO_INCREMENTAL_COST`, and failure states. Do not reduce lifecycle to a misleading active boolean."
  - Status: PASS
  - Code: `packages/provisioning/src/lifecycle.ts` — `TenantState` (25), `TRANSITIONS` (69 declared edges), `REQUIRES_APPROVAL`, `REQUIRES_OWNER`, `SERVING`, `TERMINAL`, `RESIDUAL_COST` (derived from `RESIDUAL_CLAIMS`), `nextStates`, `canAdvance`, `needsApproval`, `advance`, `attemptFor`, `ALL_STATES`, `LifecycleError`. Exported from `packages/provisioning/src/index.ts`. No new production code: this row is the requirement's own sentence, tested.
  - Caller: `advance` <- `advanceTenant` (`apps/system-studio/src/lib/registry.ts:722`, which writes the resulting state onto the `STATE` row under `ConditionExpression: "#s = :expected"` against the previous state NAME) <- `apps/system-studio/src/lib/command-handlers.ts:506` <- the tenant lifecycle controls and `POST /api/aws/operations`. Every lifecycle move in the console goes through it.
  - Tests: `apps/system-studio/e2e/lifecycle-states-logic.spec.ts` (new) — 7 cases, no browser and no server. It does NOT restate `packages/provisioning/src/provisioning.test.ts`, which already proves the graph's own properties (the happy path walks, a jump is refused, every state is reachable from DRAFT, all 69 edges are declared and the count is pinned); those 470 cases are consumed rather than duplicated.
  - **What was missing was the sentence, not the machine.** The existing suite proves things ABOUT the graph. It never asserted that the twenty-three names the requirement writes out are the names the engine has, that the arrow in the requirement is a sequence rather than a suggestion, or the last clause at all.
  - **The chain is a sequence, and that is a property rather than a diagram.** For every position in the eleven, the state TWO ahead is not reachable in one move — so `MIGRATING → READY` is refused because it skips `VERIFYING`, at every position and not only the famous one. A tenant that reached ACTIVE without verifying is an outage its users discover.
  - **The last clause is the only one a paper implementation can satisfy while violating.** A twenty-five state enum with a console rendering `active: true | false` over it has implemented the list and reduced the lifecycle. So it is tested by asking what the boolean would COST. Two states serve — `ACTIVE` and `IDLE` — and twenty-three do not; four of those twenty-three that an `active: false` field renders identically are `SUSPENDED_LOGICAL` (paused, full infrastructure, full bill), `HIBERNATED_ZERO_RUNTIME` (no runtime, still paying for retention), `LEGAL_HOLD` (may not be deleted at all) and `PURGED_ZERO_INCREMENTAL_COST` (gone, terminal). Their fingerprints over the two things the product ACTS on — what the state costs (`RESIDUAL_COST`) and where it may go next (`nextStates`) — are four distinct values. Two states agreeing on both would be a genuine argument for merging them; these do not.
  - **And the store keeps the name.** A helper's own test cannot see a producer that stopped using it, and a registry holding a boolean beside a 25-state enum would be invisible to every assertion above. So the spec reads `registry.ts` and pins three things: that `advanceTenant` calls the real `advance`, that the `STATE` write is conditional on `current.state` by name (only possible because the name is what is kept), and that no `active: true|false` appears anywhere in the registry.
  - **The one boolean in the estate is a projection and is documented as one.** `Institution.serving` in `apps/web/prisma/schema.prisma:72` is the CELL's routing switch — "whether this cell may resolve the tenant for users", written by the reconciler from the signed deployment manifest, deliberately with no default so a row created without deciding fails. It is lossy by construction (23 states collapse to `false`) and it is not where the lifecycle lives; the lifecycle lives in the control plane's registry, which is what the third test pins. A cell that knew all twenty-five states would be a cell that could be told to purge itself.
  - Determinism: no clock and no random source anywhere in the module; `advance` takes the actor's `at` as a parameter.
  - Proven by mutation, 3 of 3 caught, each restored and re-run green: `MIGRATING`'s edge list gaining `"READY"` -> 1 failed; hibernation's residual note replaced with suspension's verbatim -> 1 failed; the registry's `:expected` value no longer reading as the pinned expression -> 1 failed. Restored each time to `7 passed`; both `packages/provisioning` files verified byte-identical afterwards (`git diff --stat` lists neither).
  - Evidence: `cd apps/system-studio && npx playwright test e2e/lifecycle-states-logic.spec.ts --reporter=line` -> `7 passed (8.4s)`. `cd apps/web && npx jest --ci --testPathPattern "packages/provisioning"` -> `Test Suites: 13 passed, 13 total / Tests: 470 passed, 470 total`. Combined with the studio: `--testPathPattern "system-studio|packages/provisioning"` -> `Test Suites: 103 passed / Tests: 3427 passed`.
  - **Honest limit — this closes GE-102-001 and none of its neighbours.** The states exist, the edges are declared and the transitions are enforced and persisted. What each state's work actually DOES to AWS is GE-102-005 and GE-103-005..011, and none of it can run without the Organization.
...
- [x] **GE-103-019** — "Make clear that a purged tenant has no recoverable content; it can only be onboarded anew from independently retained configuration/customer import."
  - Status: PASS
  - Code: `apps/system-studio/src/lib/purge-finality.ts` (new) — `ContentStanding` (`intact | being-destroyed | gone`), `RebuildInput` (`what`, `short`, `from`, `heldByPlatform`), `PurgeFinality`, `INPUTS_AFTER_DESTRUCTION`, `purgeFinality`, `purgeFinalitySentence`.
  - Caller: two surfaces, one before the move and one after it. BEFORE — `apps/system-studio/src/lib/tenant-state.ts:251`, where `riskOf` appends `purgeFinalitySentence(to)` to `HighRisk.reversibility`; that risk is built at `apps/system-studio/src/app/tenants/[slug]/page.tsx:1958` for every permitted move and rendered by `DangerZone`, and `highRiskVerdict` (`tenant-state.ts:306`) recomputes it with the same function so the gate's digest still reproduces. AFTER — `apps/system-studio/src/app/tenants/[slug]/page.tsx:1838`, a `data-testid="purge-finality"` block on the "What can happen next" card, rendering the headline, the rebuild sentence, the three inputs and what the Parent keeps.
  - Tests: `apps/system-studio/e2e/purge-finality-logic.spec.ts` — 13 cases, no browser and no server.
  - **What was already said, and why it is a different sentence.** `riskOf` wrote `IRREVERSIBLE. No path back to a serving state exists from PURGING.` That is a claim about the TRANSITION GRAPH. `canReachServing`'s own comment explains that the graph edge `PURGING → FAILED → DRAFT → … → ACTIVE` is real and is not a recovery, because "rebuilding from DRAFT under the same slug produces a new, empty tenant" — the exact fact this requirement asks to be made clear, living in a code comment where no operator could read it. Now it is rendered.
  - **Three standings, because collapsing any two costs something.** `PURGE_PENDING` is `intact` — nothing has been destroyed, the decision can still be taken back, and telling an operator here that the content is gone is how a recoverable tenant gets abandoned; it deliberately carries NO rebuild inputs, because advice about an unnecessary rebuild reads as a necessary one. `PURGING` is `being-destroyed` — a half-purged tenant is not a tenant that can be talked back into service, so its recovery answer is already the same as gone. `PURGED_ZERO_INCREMENTAL_COST` is `gone`. Every other state returns `null`: a disclosure that appeared on an ACTIVE tenant is noise, and noise is how the ones that matter stop being read. Asserted exhaustively over `ALL_STATES`.
  - **`heldByPlatform` is the check, not a decoration.** The requirement's second clause is "**independently** retained". Each input names where it comes from and whether Tenure still holds it, and every input for a purged tenant answers `false` — an input the platform still held would make the first clause of the sentence untrue. The three are the configuration (a portable bundle from `exportBundle`, `lib/portability/bundle.ts`, or the customer's own copy), the customer's records (the export they took while the tenant existed, or their own systems), and the directory the people come from (their IdP; the tenant's own user pool is destroyed with everything else). None of them is "restore from backup", because GE-103-014 requires the purge to expire the backups.
  - **What the Parent keeps is read from the tombstone, not written a second time.** `parentRetains` interpolates `TOMBSTONE_FIELDS` from `@tenure/provisioning`, so the paragraph a human trusts and the key set `tombstoneProblems` enforces (GE-103-015) cannot drift. A hand-written list of five field names would be wrong the first time the tombstone changed shape, in the sentence whose whole job is to be believed.
  - **Appended, never prepended.** `DangerZone.classifyConsequence` reads the first word of `reversibility` and THROWS on anything that is neither `IRREVERSIBLE` nor `Reversible` — prepending would have broken every purge control at render time, silently. There is a test that calls `classifyConsequence` on the mutated strings for exactly that reason.
  - **`short` exists for a boundary, not for style.** The one-sentence form is appended to `reversibility`, which `DangerZone` renders into markup that `DangerZone.test.tsx` asserts against verbatim; React escapes `'` to `&#x27;`, so a possessive in the appended list failed another requirement's test for a reason belonging to neither. The long `what` keeps its apostrophes and is rendered as a React child, where the escaping is invisible.
  - Proven by mutation, 3 of 3 caught, each restored and re-run green: the configuration input's `heldByPlatform: false` -> `true` -> 1 failed; `purgeFinalitySentence(to)` -> `""` -> 2 failed; `purgeFinality(tenant.state)` -> `null` -> 1 failed. Restored each time to `13 passed`.
  - Evidence: `cd apps/system-studio && npx playwright test e2e/purge-finality-logic.spec.ts --reporter=line` -> `13 passed`. `cd apps/web && npx jest --ci --testPathPattern "system-studio"` -> `Test Suites: 90 passed, 90 total / Tests: 2957 passed, 2957 total` (`DangerZone.test.tsx` renders the mutated consequence string and passes). `npm run studio:type-check` -> no error in any file this row touches.
  - **Honest limit — this is a disclosure, not a purge.** It says what is true of a purged tenant. Whether the purge actually deleted everything is GE-103-014, and no purge can run today (`REFUSED_OPERATIONS`, and GE-103-015's row records that nothing writes a tombstone). So today the panel is reachable in the console for a tenant an operator moves into a purge state, and the sentence it prints will be the one that matters on the day a purge does run.
