...
matches is therefore digested on both sides: 254 shared paths, 88
identical, **166 divergent**, 0 undetermined. Digest is
...
| Events / scheduled jobs | 2 | 7 | 0 | 2 | 1 | `MERGE_REQUIRED` | every source path is present in the target tree, but 1 of 2 shared path(s) differ in content, so the target does not hold the source implementation | `apps/web/src/app/api/jobs/reminders/route.ts` |
...
20 workspaces across both repositories: MERGE_REQUIRED 1, PARENT_CANONICAL 19.
...
| `@tenure/generality-fixtures` | — | `packages/generality-fixtures/package.json` | `PARENT_CANONICAL` | a target-only workspace; the source has nothing to merge | — |
